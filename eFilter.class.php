...
public function parseTpl ($array1, $array2, $tpl): array|string
{
    return str_replace(search: $array1, replace: $array2, subject: $tpl);
}
            ...
            if (isset($filter_values_full[$tv_id])) {
                ...
                if(
                    ...
                    continue;
                                ...
                                // Пример: получение максимальной цены из фильтруемых значений
                $maxvalcurr = max(array_keys($filter_values_full[$tv_id])); // Получаем максимальную цену из всех доступных
                ...
                switch ($filters[$tv_id]['type']) {
                    ...
                    case '9'://одиночный чекбокс
                        ...
                        foreach ($filter_values_full[$tv_id] as $k => $v) {
                            ...
                            // Здесь добавляем условие для проверки значения count
                            if ($disabled == '') {
                                $count = (isset($filter_values[$tv_id][$k]['count']) ? $filter_values[$tv_id][$k]['count'] : $filter_values_full[$tv_id][$k]['count']);
                            } else {
                                $count = $this->zero;
                            ...
                            // Условие для фильтрации по count
                            if ($count > 0) {
                                if ($this->params['removeDisabled'] == '0' || $disabled == '') {
                                    $i++;
                                    $wrapper .= $k != '' ? $this->parseTpl(
